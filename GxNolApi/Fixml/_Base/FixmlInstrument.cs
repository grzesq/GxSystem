...
namespace GxNolApi.NolClient.Fixml
{
	...
	public class FixmlInstrument
	{
		...
		{
			if (string.IsNullOrEmpty(isin)) return FindBySym(symb);
			if (string.IsNullOrEmpty(isin)) return FindById(isin);
		...
		public void Write(XElement parent, string name)
		{
			XElement elem = new XElement("Instrmt", new XAttribute("Sym", Symbol));
			if (SecurityId != null)
			{
                elem.Add(new XAttribute("ID", SecurityId)); 
				elem.Add(new XAttribute("Src", isinSrc.ToString()));
			}
			parent.Add(elem);
		}

		public static FixmlInstrument Read(XElement parent, string name)
		{
			XElement elem = parent.FirstNode as XElement;
